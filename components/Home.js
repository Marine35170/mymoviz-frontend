  ...
  useEffect(() => {
   ...
   .then( data =>{
    const formattedMovies = data.movies.map(movie => {
      // Ajout du préfixe à l'URL de l'image
      const poster = `https://image.tmdb.org/t/p/w500/${movie.poster_path}`;

      // Tronquer l'overview à 250 caractères si nécessaire
      let overview = movie.overview;
      if (overview.length > 250) {
        overview = `${overview.substring(0, 250)}...`;
      }

      // Retourner le film avec les données formatées
      return {
        title: movie.title,
        poster: poster,
    ...
    setMoviesData(formattedMovies); // Mise à jour des données
  })
   .catch(error => console.error('Oups, erreur de chargement de votre mouviz'))
  }, []);
